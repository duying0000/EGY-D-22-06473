...
im_data = np.load('im_sg_cwt.npy')
spec_data = np.load('spc_LAI.npy')

im = np.load('im.npy')
im_num = np.arange(len(im_data))

inp_spc = keras.Input(shape=(128, 1))
inp_im = keras.Input(shape=(28, 28, 3))
model = merge_model(inp_spc, inp_im, N1)
model.summary()

# model = mode_2D_a(28,28,128,N1)
# model.summary()
#
# model = mode_3D(28,28,128,N1)
# model.summary()
...
valiY = valiSpc[:, -1]
testY = testSpc[:, -1]
trainSpcX = mean_spec(im_data[trainImagesX_n,:,:,:])
testSpcX = mean_spec(im_data[testImagesX_n,:,:,:])
valiSpcX = mean_spec(im_data[valiImagesX_n,:,:,:])
train_im = im[trainImagesX_n,:,:,:]
test_im = im[testImagesX_n,:,:,:]
vali_im = im[valiImagesX_n,:,:,:]
trainImagesX1 = train_im[:,:,:,[58, 38, 25]]
testImagesX1 = test_im[:,:,:,[58, 38, 25]]
valiImagesX1 = vali_im[:,:,:,[58, 38, 25]]
...
# h1 = model.fit(trainImagesX, trainY, epochs=256, batch_size=128, validation_data=(valiImagesX, valiY),
#                callbacks=[early_stopping],verbose=2)
